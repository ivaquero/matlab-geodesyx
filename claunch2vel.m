% Coordinate Transformation: Launch ==> Velocity
%
% Syntax:
%   varargout = claunch2vel(launch, launch_vel, lat0, lon0, alt0, options)
%
% Input:
%   launch                          Target's launch coordinates in [m, m, m]
%   launch_vel                      Target's launch velocit in [m/s, m/s, m/s]
...
function varargout = claunch2vel(launch, launch_vel, lift, side_force, lat0, lon0, alt0, options)

    arguments
        launch (3, 1) double
        launch_vel (3, 1) double
        lift (3, 1) double
        side_force (3, 1) double
        lat0 {mustBeReal}
        lon0 {mustBeReal}
        alt0 {mustBeReal}
    ...
    [roll, pitch, yaw] = unedv2euler(launch_vel, lift, side_force);
    ...
    velocity = launch2vel * launch + ECEF_g;

    if nargout == 1
        varargout{1} = velocity;
    elseif nargout == 3
        varargout{1} = velocity(1);
        varargout{2} = velocity(2);
        varargout{3} = velocity(3);
    else
